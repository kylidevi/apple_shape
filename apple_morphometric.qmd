...
def gpa_mean(apple_arr, landmark_num, dim_num):
    ...
    ref_shape = apple_arr[ref_ind, :, :] # select the reference shape
    ...
    while d > mean_diff: # set boolean criterion for Procrustes distance between mean to stop calculations
        ...
        arr = np.zeros( ((len(apple_arr)),landmark_num,dim_num) ) # empty 3D array: # samples, landmarks, coord vals

        for i in range(len(apple_arr)): # for each apple shape 

            s1, s2, distance = procrustes(old_mean, apple_arr[i]) # calculate procrustes adjusted shape to ref for current apple
...
<!-- Following sections are for reading in the data and preliminary image analysis -->
...
mdata = pd.read_csv("apple_metadata.csv") 
...
# set image directory
data_dir = "all_binaries/binary_side_domestica/" 
...
-   Select the contour of the largest object (the apple)
...
-   Rotate apples and scale to centimeters
...
-   `res` is the desired number of points to interpolate on each side of the apple
...
# an initial high resolution outline of the apple
high_res_pts = 1000 
...
for lf in range(len(mdata)):
    ...
    print(lf, curr_image) # print each apple in case there are problems later
...
```{python}
#| label: Pseudo-landmarks (Figure 1d)
# Plot each apple
...
# export figure (panel 1d)
plt.savefig("Figure1d.pdf")
```

```{python}
#| label: Pseudo-landmarks shape (Figure S1)
# Plot each apple
...
Using placed pseudo-landmarks representing apples that are rotated upwards and scaled in centimeters from `apple_cm_arr`, calculate the following:
...
mdata.to_csv("analysis_apple_metadata.csv", index = False)
```
...
# reading data
apple_traits <- read.csv("analysis_apple_metadata.csv")
...
# save file
write.csv(apple_traits, file = "analysis_apple_metadata_avg.csv")
```
...
# load in data
reml_design <- read.table("20161024_ABC_design_grid_zm.txt", header = T)

apple_traits <- read.csv("analysis_apple_metadata_avg.csv")
...
-   Calculate a PCA for just the axes needed for reconstruction of eigenapples for morphospace (probably 2)
...
new_mdata = pd.read_csv("analysis_apple_metadata.csv")
...
# save output with PC
new_mdata.to_csv("analysis_apple_metadata.csv", index = False)
```
...
```{r}
#| label: Pair Plot by Cultivar (Figure S2)
library(tidyverse)
library(GGally)
library(readxl)
library(ggthemes)
...
# import data
predicted <- read.delim("Cprediction_values.txt", sep = "")
...
# This is TableS1
write.csv(all_data, file = "TableS1.csv")
...
# save summary
write.csv(trees, file = "sample_size.csv", row.names = FALSE)
...
# save plot
ggsave("FigureS2.pdf", plot = plot1, units = "in", width = 17, height = 15)
```
...
new_mdata = pd.read_csv("_metadata.csv")
...
for i in range(len(PC_vals)):
    ...
    inv_apple = pca.inverse_transform(np.array(PC_vals[i]))
...
averaged = pd.read_csv("analysis_apple_metadata.csv")
...
for i in PC1_vals:  # for each PC1 interval
    for j in PC3_vals:  # for each PC2 interval
        ...
        # calculate the inverse eigenapple
        inv_apple = pca.inverse_transform(np.array([pc1_val, 0, pc3_val]))
        inv_x = inv_apple[0::2]  # select just inverse x vals
        inv_y = inv_apple[1::2]  # select just inverse y vals

        # plot the inverse eigenapple
