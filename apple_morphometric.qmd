...
mdata = pd.read_csv("C:/Users/User/Desktop/COOP02/code/apple_shape/apple_metadata.csv") 
...
mdata.to_csv("20241029apple_metadata.csv", index = False)
```
...
apple_traits <- read.csv("20241029apple_metadata.csv")

# separating the nursery id from the name
apple_traits <- apple_traits %>% 
  separate(file, into = c("BW", "nursery_id", "ending"), sep = "_", remove = FALSE) %>% 
  dplyr::select(2, 4, 6:18)
...
# save file
write.csv(apple_traits, file = "20241029apple_metadata_avg.csv")
```
...
apple_traits <- read.csv("20241029apple_metadata_avg.csv") %>% 
  dplyr::select(2:16)
...
new_mdata = pd.read_csv("20241029apple_metadata.csv")
...
new_mdata.to_csv("20241029apple_metadata.csv", index = False)
```
...
# separating the nursery id from the names
apple_traits <- apple_traits %>% 
  separate(file, into = c("BW", "nursery_id", "ending"), sep = "_", remove = FALSE) %>% 
  dplyr::select(2, 4, 6:27)

# averaging within the tree
apple_traits <- apple_traits %>% 
  group_by(nursery_id) %>% 
  mutate(across(all_of(c(21:23)), ~mean(.x, na.rm = TRUE), .names = "{col}"))
...
# save data before distinct
write.csv(all_data, file = "20241029apple_metadata.csv")
...
# 47 NA values in weight_avg_16_harv ASK ABOUT THIS
plot1 <- ggpairs(all_data, columns = 19:28, aes(fill = "grey", alpha = 0.5), columnLabels = c("Width", "Length", "Area", "Solidity", "Asymmetry", "Aspect Ratio", "Harvest Weight", "PC1", "PC2", "PC3"), upper = list(continuous = "points"), diag = list(continuous = wrap("densityDiag", fill = "black")))

# save plot
ggsave("SupplementalFigure1_draft1029.pdf", plot = plot1, units = "in", width = 17, height = 15)
```

### Spearman's Rank Correlation and Bonferroni Correction

```{r}
#| label: Spearman's Rank Correlation and Bonferroni Correction

# Spearman's Rank Correlation
all_data <- read.csv("20241029apple_metadata.csv")
...
write.csv(pairwise_comp, file = "bonferroni_pval.csv")
write.csv(pairwise_comp_r, file = "bonferroni_rho.csv")
```
...
new_mdata = pd.read_csv("20241004apple_metadata.csv")
...
for i in PC1_vals:  # for each PC1 interval
    for j in PC2_vals:  # for each PC2 interval
        ...
        inv_apple = pca.inverse_transform(np.array([pc1_val, pc2_val, 0]))
...
plt.savefig("Figure5_draft1004.pdf")
```
...
averaged.to_csv("20241004apple_metadata.csv", index = False)
```
