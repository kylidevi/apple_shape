...
def polygon_perimeter(vertices):
  ...
  Args:
    vertices: A numpy array of shape (n, 2) representing the coordinates
              of the polygon's vertices, where n is the number of vertices.
  ...
  perimeter += np.sqrt(np.sum((vertices[0] - vertices[-1])**2))
...
mdata = pd.read_csv("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/apple_metadata.csv") 
...
# set image directory
data_dir = "C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/binary_domestica/" 
...
# export figure
plt.savefig("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505Figure1d.pdf")
```

```{python}
#| label: Pseudo-landmarks shape
# Plot each apple
...
for i in range(20):
    ...
    # find the left most point (MIN X)
    min_x = np.min(apple_cm_arr[i,:,0])
    min_x_index = np.argmin(apple_cm_arr[i,:,0])
    min_y = apple_cm_arr[i, min_x_index, 1]
    plt.scatter(min_x, min_y, c="red", s=3)
    
    # find the right most point (MAX X)
    max_x = np.max(apple_cm_arr[i,:,0])
    max_x_index = np.argmin(apple_cm_arr[i,:,0])
    max_y = apple_cm_arr[i, max_x_index, 1]
    plt.scatter(max_x, max_y, c="blue", s=3)
    
    # find the topmost point (MAX Y)
    max_y = np.max(apple_cm_arr[i,:,1])
    max_y_index = np.argmin(apple_cm_arr[i,:,1])
    max_x = apple_cm_arr[i, max_y_index, 0]
    plt.scatter(max_x, max_y, c="orange", s=3)
    
    # find the lowermost point (MIN Y)
    min_y = np.min(apple_cm_arr[i,:,1])
    min_y_index = np.argmin(apple_cm_arr[i,:,1])
    min_x = apple_cm_arr[i, max_y_index, 0]
    plt.scatter(min_x, min_y, c="purple", s=3)
...
# export figure
plt.savefig("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505supplementalLandmarks.pdf")
```
...
for lf in range(len(apple_cm_arr)):
    ...
    perimeter = polygon_perimeter(curr_lf) # calculate perimeter
...
mdata["circularity"] = circularity_list

# save output so you don't have to run the code all the time
mdata.to_csv("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505apple_metadata.csv", index = False)
```
...
# reading data
apple_traits <- read.csv("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505apple_metadata.csv")

# averaging within the tree
apple_traits <- apple_traits %>% 
  group_by(nursery_id) %>% 
  mutate(across(all_of(c(8:12)), ~mean(.x, na.rm = TRUE), .names = "{col}"))

# save file
write.csv(apple_traits, file = "C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505apple_metadata_avg.csv")
```
...
# load in data
reml_design <- read.table("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/reml/20161024_ABC_design_grid_zm.txt", header = T)

apple_traits <- read.csv("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505apple_metadata_avg.csv")
...
### CIRCULARITY ---------
#Run the model
circularity_reml <- lmer(formula = circularity ~ apple_id + ( 1 | Block) + ( 1 | Block:rGrid) + ( 1 | Block:cGrid) + ( 1 | Block:cGrid:rGrid), data= ph,  REML = TRUE) #apple_id is fixed effect

#Get the least-square means
circularity_reml_lsmeans <- lsmeans(circularity_reml , "apple_id")
circularity_reml_lsmeans_predict <- predict(circularity_reml_lsmeans)
names(circularity_reml_lsmeans_predict) = levels(ph$apple_id)

# Adding to the data
predict <- cbind(predict, circularity_reml_lsmeans_predict)

#Compare the predicted ls_means by cultivar to a simple arithmetic mean by cultivar
mean_vals = aggregate(circularity ~ apple_id, data = ph, mean)
plot(mean_vals$circularity, circularity_reml_lsmeans_predict)
cor.test(mean_vals$circularity, circularity_reml_lsmeans_predict)
#cor circularity: 0.9909962

predict <- tibble::rownames_to_column(predict, "apple_id")
write_delim(predict, file = "C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/prediction_values.txt")
```
...
new_mdata = pd.read_csv("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505apple_metadata.csv")
...
# save output with PC
new_mdata.to_csv("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505apple_metadata.csv", index = False)
```
...
apple_traits <- apple_traits %>% 
  group_by(nursery_id) %>% 
  mutate(across(all_of(c(13:15)), ~mean(.x, na.rm = TRUE), .names = "{col}"))
...
# import data
predicted <- read.delim("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/prediction_values.txt", sep = "")
apple_data <- read.csv("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505apple_metadata.csv") 
dataset <- read_excel("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/Dataset_S1.xlsx", sheet = "phenotype_data")
...
all_data <- all_data %>% relocate(aspect_ratio, .after = circularity_reml_lsmeans_predict)
all_data <- all_data %>% relocate(weight_avg_16_harv, .after = aspect_ratio)

# save data before with all apple id
write.csv(all_data, file = "C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505apple_metadata.csv")
...
# save summary
write.csv(trees, file = "C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505sample_size.csv", row.names = FALSE)
...
plot1 <- ggpairs(all_data, columns = c(17:26, 34), aes(fill = "grey", alpha = 0.5), columnLabels = c("Width", "Length", "Area", "Solidity", "Circularity", "Aspect Ratio", "Harvest Weight", "PC1", "PC2", "PC3", "Release Year"), upper = list(continuous = "points"), diag = list(continuous = wrap("densityDiag", fill = "black"))) + theme_few()

# save plot
ggsave("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/SupplementalFigure1_0505.pdf", plot = plot1, units = "in", width = 17, height = 15)
```
...
# save figure
plt.savefig("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/SupplementalFigure2_0505.pdf")
```
...
averaged = pd.read_csv("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/20250505apple_metadata.csv")
...
# save figure
plt.savefig("C:/Users/User/Desktop/CO-OP/COOP02/code/apple_shape/output_final/Figure3_0505.pdf")
```
